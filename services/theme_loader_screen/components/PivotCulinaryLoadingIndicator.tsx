...
type PivotCulinaryLoadingIndicatorProps = React.ComponentProps<
  typeof AnimatedSvg
> & {
  ref?: Ref<Svg>;
  size: number;
...
export default function PivotCulinaryLoadingIndicator(
  props: PivotCulinaryLoadingIndicatorProps
) {
  ...
  >(() => {
    const halfClose = interpolate(
      transition.value,
      [0.4, 1],
      [VIEWBOX_HEIGHT, 0]
    );
    const halfOpen = interpolate(
      transition.value,
      [0, 0.6],
      [VIEWBOX_HEIGHT, 0]
    );
