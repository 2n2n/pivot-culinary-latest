...
export default function GamedayLoadingIndicator(props: GamedayLoadingIndicatorProps) {
    ...
    const animatedPathProps = useAnimatedProps<React.ComponentProps<typeof AnimatedPath>>(() => ({
        ...
        strokeDasharray: `${STROKE_LENGTH} ${interpolate(transition.value, [0, 0.4], [0, STROKE_LENGTH], Extrapolation.CLAMP)}`,
        strokeDashoffset: interpolate(transition.value, [0.4, 1], [STROKE_LENGTH, 0], Extrapolation.CLAMP),
    }));
    useEffect(() => {
        transition.value = withRepeat(withTiming(1, { duration: 1500 }), -1);
        return () => {
            transition.value = 0;
        }
    }, []);
    return <AnimatedSvg height={size} width={size * VIEWBOX_RATIO} viewBox={VIEWBOX_VALUE} fill="none" ref={ref} {...svgProps}>
        ...
        <AnimatedPath 
            ...
            d="M173 265H395L295 470.5C207.5 470.5 60 441 60 265 60 122 151.5 60.5 241.5 60.5H653C757.5 60.5 849.5 131.5 849.5 265 849.5 398.5 740 470.5 653 470.5 583.4 470.5 416.74 470.901 411.523 470.901"  
            stroke="white"
