...
} & Pick<React.ComponentProps<typeof VStack>, "className" | "style" | "space" | "reversed">

const EventDetails: EventDetailsComponent = ({event, pending = false, showFinance = false, showBeo = false, children, ...stackProps}) => {
    if (!event) return null;
    return (
        <EventDetailsContext.Provider value={{ event, showFinance, showBeo, pending }}>
            <VStack className={twMerge("gap-2 px-4", stackProps.className)} {...stackProps}>
                {children}
            </VStack>
