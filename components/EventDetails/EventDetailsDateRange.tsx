...
const EventDetailsDateRange: React.FC<DateRangeProps> = (props = {}) => {
    ...
    if (sameDay || !formattedEndDate) return <HStack className={twMerge("gap-2 items-center", props.stackClassName)}>
        <Icon as={CalendarDays} className={twMerge("text-pivot", props.iconClassName)} />
        <Text className={twMerge(props.textClassName)}>{formattedStartDate}</Text>
    </HStack>;
    return <HStack className={twMerge("gap-2 items-center", props.stackClassName)}>
        <Icon as={CalendarDays} className={twMerge("text-pivot", props.iconClassName)} />
        <Text className={twMerge(props.textClassName)}>{formattedStartDate} - {formattedEndDate}</Text>
    </HStack>
};
