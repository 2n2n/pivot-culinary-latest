...
export default function BookingDetails() {
  ...
  const beo = useMemo(() => {
    if (bookingData && bookingData.documents)
              ...
              <VStack className="gap-4">
                ...
                <VStack className="gap-2 px-4">
                  ...
                  <VStack className="mt-10 mb-5 gap-2">
                    ...
                    <Button
                      disabled={beo === null}
                      className="rounded-full disabled:bg-slate-400"
                      onPress={() => {
...
export const BookingEventCard = ({ event }: { event: TripleseatEvent }) => {
  ...
  return (
    <Box key={event.id} className="bg-white p-4 rounded-[8px]">
        ...
        <VStack className="flex-1">
          <HStack className="justify-between items-center">
            ...
            <HStack className="gap-1">
              ...
              <Text className="text-sm text-gray-700">{event.guest_count}</Text>
            </HStack>
          </HStack>
      ...
      <HStack className="justify-end gap-3 mt-4">
        <Button
          ...
          className={`px-4 py-2 rounded-full ${
            !beo ? "border-gray-600" : "border-primary-500"
          }`}
        >
          <Icon
            as={NotepadText}
            className={`w-4 h-4 ${!beo ? "text-gray-600" : "text-primary-500"}`}
          />
          <ButtonText
            disabled={!beo}
            className={`font-bold disabled:text-gray-600`}
            onPress={() => {
              if (beo) Linking.openURL(beo?.url || "");
            }}
          >
...
const BookingEvents = ({ bookingId }: { bookingId: string }) => {
  ...
  const { orderedDates, groupedBookingEvents } = useMemo(() => {
    ...
    const updatedGroupedBookingEvents: Record<
      string,
      Array<TripleseatEvent>
    > = {};

    if (!bookingEventsQuery.data?.length)
      return {
    ...
    <>
      ...
      <VStack className="bg-gray-100 px-4 py-4 gap-4">
        {selectedEvents.map((event) => (
          <BookingEventCard event={event} key={event.id} />
        ))}
      </VStack>
    </>
  );
      ...
      <HStack className="gap-4 px-4">
        {dates.map((item, index) => (
          <Button
            key={item}
            variant={
              item === activeDate || (index === 0 && !activeDate)
                ? "solid"
                : "link"
            }
...
const EmptyBookingEvents = () => (
  <VStack className="bg-white py-2 gap-1 h-[400px]">
    <Text className="text-xl font-bold px-4">Events</Text>
